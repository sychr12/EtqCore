...
let commonPrintBusy = false;
document.querySelector('#printCommon').onclick = async () => {
  if (commonPrintBusy || !form.reportValidity()) return;
  const count = Number(document.querySelector('#labelCount').value);
  if (!Number.isInteger(count) || count < 1 || count > 1000) { status('Informe de 1 a 1000 etiquetas.', 'error'); return; }
  const printWindow = window.open('', '_blank');
  if (!printWindow) { status('Permita abrir uma nova janela para imprimir.', 'error'); return; }
  commonPrintBusy = true;
  document.querySelector('#printCommon').disabled = true;
  printWindow.document.title = 'Imprimir etiquetas';
  printWindow.document.body.textContent = 'Preparando etiquetas…';
  setBusy(true, 'Preparando impressão', 'Gerando etiquetas para impressora comum');
  try {
    const preview = await updatePreview();
    if (!preview) throw new Error('Confira os dados da etiqueta antes de imprimir.');
    const template = document.querySelector('#previewLabel').cloneNode(true);
    const response = await fetch('/api/gerar', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...values(), destino: 'impressora_comum', quantidade_etiquetas: count })
    });
    const result = await response.json();
    if (!response.ok) throw new Error(result.erro || 'Não foi possível gerar as etiquetas.');
    await loadState();
    await loadHistory();
    if (printWindow.closed) throw new Error('A janela foi fechada. As etiquetas geradas estão no histórico.');
    const doc = printWindow.document;
    doc.body.replaceChildren();
    const link = doc.createElement('link');
    link.rel = 'stylesheet'; link.href = new URL('/static/style.css?v=78', location.href).href;
    const ready = new Promise((resolve, reject) => { link.onload = resolve; link.onerror = () => reject(new Error('Não foi possível carregar o layout de impressão.')); });
    doc.head.append(link);
    const style = doc.createElement('style');
    style.textContent = `@page { size: A4; margin: 10mm; }
      html, body { margin: 0; padding: 0; background: white; }
      .printHelp { padding: 16px; font: 14px Arial; color: #173b56; }
      .paperLabel { break-inside: avoid; page-break-inside: avoid; padding: 0 0 5mm; }
      .paperLabel .label { width: ${Number(cfg.largura_mm)}mm !important; height: ${Number(cfg.comprimento_mm)}mm !important; max-width: none; border: 2px solid black; box-shadow: none; border-radius: 0; }
      .label, .label * { print-color-adjust: exact; -webkit-print-color-adjust: exact; animation: none !important; transition: none !important; }
      .label .qr, .label .qr img { opacity: 1 !important; }
      @media print { .printHelp { display: none; } }`;
    doc.head.append(style);
    const help = doc.createElement('div'); help.className = 'printHelp';
    help.textContent = 'Selecione sua impressora comum. Use papel A4, escala 100% e desative cabeçalhos e rodapés. As etiquetas já foram registradas no histórico; cancelar esta janela não devolve os números. ';
    const button = doc.createElement('button'); button.textContent = 'Imprimir'; button.onclick = () => printWindow.print();
    help.append(button); doc.body.append(help);
    for (const item of result.etiquetas) {
      const label = template.cloneNode(true);
      label.removeAttribute('id'); label.querySelectorAll('[id]').forEach(el => el.removeAttribute('id'));
      label.querySelectorAll('*').forEach(el => el.style.removeProperty('font-size'));
      const qrImage = doc.createElement('img'); qrImage.alt = `QR ${item.identificador}`;
      qrImage.src = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(item.qr_svg);
      label.querySelector('.qr').replaceChildren(qrImage);
      label.querySelectorAll('img:not(.qr img)').forEach(img => { img.src = new URL(img.getAttribute('src'), location.href).href; });
      const sheet = doc.createElement('div'); sheet.className = 'paperLabel'; sheet.append(label); doc.body.append(sheet);
    }
    await ready;
    await Promise.all([...doc.images].map(img => img.decode()));
    await doc.fonts.ready;
    doc.querySelectorAll('.label').forEach(fitLabelTexts);
    status(`${count} etiqueta(s) preparada(s) e registrada(s). Selecione a impressora na janela de impressão.`, 'ok');
    printWindow.focus(); printWindow.print();
  } catch (error) {
    status(error.message, 'error');
    if (!printWindow.closed) printWindow.document.body.textContent = error.message;
  } finally {
    setBusy(false); commonPrintBusy = false;
    document.querySelector('#printCommon').disabled = false;
  }
};
