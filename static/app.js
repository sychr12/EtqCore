...
function fitLabelTexts(root) { if (!root) return; root.querySelectorAll('.badgeValue,.dataTable .cellValue').forEach(v => fitText(v, v.parentElement.clientWidth * 0.92)); const content = root.querySelector('.content'); fitText(root.querySelector('.titleText'), content.clientWidth * 0.94, 22); const codMain = root.querySelector('.codMain'), codMainLabel = root.querySelector('.codLabelMain'), codValueMain = root.querySelector('.codValueMain'), codStack = root.querySelector('.codStack'), codValue = root.querySelector('.codValue'); if (codMain && codValueMain) fitText(codValueMain, Math.max(40, codMain.clientWidth - (codMainLabel?.getBoundingClientRect().width || 0) - 28), 18); if (codStack && codValue) fitText(codValue, Math.max(40, codStack.clientWidth - 22), 18); const medValue = root.querySelector('.medValue'), medContent = root.querySelector('.medContent'); if (medValue && medContent) fitText(medValue, medContent.clientWidth * 0.98, 16) }
