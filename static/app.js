 ...
 */
let cfg = {}, timer, qrUrl, historyCache = {}, lastPrintProfile = null, previewRevision = 0;
let generationPending = false, confirmationPending = false, historyRevision = 0;
...
async function loadState(restoreLast = false) {
  const r = await fetch('/api/estado'), s = await r.json(), previousBranch = cfg.filial;
  if (r.ok === false) throw new Error(s.erro || 'Não foi possível consultar o estado do sistema.');
  cfg = s.config;
  updateDeleteQuantity();
  document.querySelector('#nextTop').textContent = 'Próximo: ' + s.proximo_numero;
  const branch = document.querySelector('#qrBranch');
  if (previousBranch === undefined || branch.value === previousBranch) branch.value = cfg.filial;
  document.querySelector('#qrIdentifier').value = s.proximo_identificador;
  for (const [k, v] of Object.entries(cfg)) {
    const el = document.querySelector(`#settingsForm [name="${k}"]`);
    if (el) el.value = v;
  }
  if (restoreLast && s.ultima_etiqueta) {
    fillForm(s.ultima_etiqueta.dados, false);
    status(`Última etiqueta ${s.ultima_etiqueta.identificador} restaurada automaticamente.`, 'info');
  }
  setText('#branchChip', 'Filial (E) ' + branch.value);
  resizePreview();
  return s;
}
async function refreshStateAndPreview() {
  clearTimeout(timer);
  ++previewRevision;
  clearQrPreview();
  await loadState();
...
function clearQrPreview() {
  lastPrintProfile = null;
  setText('#qrText', '');
  const box = document.querySelector('#qrBox');
  if (box) { box.classList.add('empty'); box.replaceChildren(); }
  if (qrUrl) { URL.revokeObjectURL(qrUrl); qrUrl = null; }
}
async function updatePreview() {
  const revision = ++previewRevision, d = values();
  clearQrPreview();
  const feedback = setText('#qrFeedback', '');
  document.querySelectorAll('[data-out]').forEach(el => { el.textContent = d[el.dataset.out] || ''; });
  const brandBand = document.querySelector('.brandBand');
  if (brandBand) brandBand.classList.toggle('isEmpty', !String(d.cliente || '').trim());
  const observationBox = document.querySelector('.observationBox');
  if (observationBox) observationBox.classList.toggle('isEmpty', !String(d.observacao || '').trim());
  setText('#branchChip', 'Filial (E) ' + d.filial);
  setText('[data-out-lot] .lotTop', d.lote_base || '');
  setText('[data-out-lot] .lotBottom', '');
  setText('[data-out-qty] .qtyNumber', d.quantidade);
  setText('[data-out-qty] .qtyUnit', d.unidade);
  setText('[data-out-date="fabricacao"]', fmtDate(d.fabricacao));
  setText('[data-out-date="validade"]', fmtDate(d.validade));
  resizePreview();
  if (!d.quantidade) return;
  try {
    const r = await fetch('/api/preview', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(d) });
    const x = await r.json();
    if (revision !== previewRevision) return;
    if (!r.ok) throw new Error(x.erro || 'Confira os campos da etiqueta.');
    const qr = await fetch('/api/qr.svg', { method: 'POST', body: x.qr });
    if (revision !== previewRevision) return;
    if (!qr.ok) throw new Error('Não foi possível gerar a imagem do QR Code.');
    ...
    if (qrBox) {
      ...
      qrBox.replaceChildren(Object.assign(document.createElement('img'), { alt: 'QR Code', src: qrUrl }));
    }
    setText('#qrText', x.qr);
    document.querySelector('#qrIdentifier').value = x.identificador;
    lastPrintProfile = x.impressao;
    resizePreview();
    return x;
  } catch (error) {
    if (revision !== previewRevision) return;
    clearQrPreview();
    if (feedback) { feedback.textContent = error.message; feedback.className = 'status error'; }
  }
}
...
form.addEventListener('input', () => { clearTimeout(timer); ++previewRevision; clearQrPreview(); timer = setTimeout(updatePreview, 250) });
...
function confirmAction(title, message, confirmText = 'Confirmar') { if (confirmationPending) return Promise.resolve(false); confirmationPending = true; return new Promise(resolve => { const modal = document.querySelector('#confirmModal'), ok = document.querySelector('#confirmOk'), cancel = document.querySelector('#confirmCancel'); document.querySelector('#confirmTitle').textContent = title; document.querySelector('#confirmMessage').textContent = message; ok.textContent = confirmText; modal.classList.add('show'); modal.setAttribute('aria-hidden', 'false'); const finish = value => { confirmationPending = false; modal.classList.remove('show'); modal.setAttribute('aria-hidden', 'true'); ok.onclick = null; cancel.onclick = null; window.removeEventListener('keydown', onKey); resolve(value) }, onKey = e => { if (e.key === 'Escape') finish(false) }; ok.onclick = () => finish(true); cancel.onclick = () => finish(false); window.addEventListener('keydown', onKey); setTimeout(() => ok.focus(), 120) }) }
function fillForm(data, refresh = true) {
  const complete = { filial: cfg.filial || '', ...data };
  // Um arquivo parcial não pode herdar tipo, DPD ou lote do produto anterior.
  for (const el of form.elements) {
    if (el.name) el.value = complete[el.name] ?? '';
  }
  clearTimeout(timer);
  ++previewRevision;
  clearQrPreview();
...
function readProject(project) {
  ...
  if (!isObject(project) || project.tipo_arquivo !== 'etiqueta-zebra-editavel' || !isObject(project.dados) || (project.versao !== undefined && project.versao !== 1)) {
    throw new Error('Este arquivo não é uma etiqueta .etq válida ou usa uma versão não suportada.');
  }
  const data = {};
  for (const el of form.elements) {
    if (!el.name || !Object.hasOwn(project.dados, el.name)) continue;
    const value = project.dados[el.name];
    if (value !== null && typeof value !== 'string' && !(typeof value === 'number' && Number.isFinite(value))) {
      throw new Error(`O campo ${el.name} do arquivo deve conter texto ou um número.`);
    }
    data[el.name] = value ?? '';
  }
  const dimensions = {};
  if (project.dimensoes !== undefined) {
    if (!isObject(project.dimensoes)) throw new Error('As dimensões da etiqueta salva são inválidas.');
    for (const key of ['largura_mm', 'comprimento_mm', 'dpi']) {
      if (!Object.hasOwn(project.dimensoes, key)) continue;
      const value = project.dimensoes[key];
      if (!['number', 'string'].includes(typeof value) || !String(value).trim() || !Number.isFinite(Number(value))) {
        throw new Error('As dimensões da etiqueta salva são inválidas.');
      }
...
document.querySelector('#openProject').onchange = async e => {
  const file = e.target.files[0];
  if (!file) return;
  setBusy(true, 'Abrindo etiqueta', 'Carregando dados salvos');
  try {
    if (file.size > 1024 * 1024) throw new Error('O arquivo é grande demais.');
    const { data, dimensions } = readProject(JSON.parse(await file.text()));
    if (Object.keys(dimensions).length) {
      const r = await fetch('/api/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...cfg, ...dimensions }) });
      if (!r.ok) throw new Error((await r.json()).erro || 'Não foi possível aplicar as dimensões da etiqueta.');
      await loadState();
    }
    fillForm(data, false);
    await updatePreview();
    status('Etiqueta aberta para edição. O contador não foi consumido.', 'ok');
  } catch (err) {
...
async function generate(dest) {
  if (generationPending || !form.reportValidity()) return;
  const count = dest === 'imprimir' ? Number(document.querySelector('#labelCount').value) : 1;
  if (!Number.isInteger(count) || count < 1 || count > 1000) { status('A quantidade de etiquetas deve ficar entre 1 e 1000.', 'error'); return; }
  generationPending = true;
  let attempted = false;
  try {
    const warning = dest === 'imprimir' ? `Serão impressas ${count} etiqueta${count > 1 ? 's' : ''}, cada uma com contador e QR próprios. Mesmo em caso de falha, os números ficarão reservados.` : 'O próximo contador será reservado permanentemente para este arquivo de impressão.';
    const confirmed = await confirmAction(dest === 'imprimir' ? 'Confirmar envio para a Zebra' : 'Gerar arquivo da impressora', warning, dest === 'imprimir' ? 'Enviar agora' : 'Gerar PRN');
    if (!confirmed) return;
    setBusy(true, dest === 'imprimir' ? 'Enviando para a Zebra' : 'Gerando arquivo PRN', dest === 'imprimir' ? `Preparando ${count} etiqueta${count > 1 ? 's' : ''} com QR exclusivo` : 'Montando etiqueta e reservando contador');
    status(dest === 'imprimir' ? 'Preparando o lote para envio…' : 'Gerando arquivo de impressão…');
    attempted = true;
    clearTimeout(timer);
    ++previewRevision;
    clearQrPreview();
    const r = await fetch('/api/gerar', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...values(), destino: dest, quantidade_etiquetas: count }) }), x = await r.json();
    if (!r.ok) {
      const range = x.quantidade > 1 ? ` Os números de ${x.identificador} até ${x.ultimo_identificador} foram consumidos por segurança.` : ` O número ${x.identificador} foi consumido por segurança.`;
      throw new Error((x.erro || 'Não foi possível gerar a etiqueta.') + (x.consumido ? range : ''));
    }
    if (dest === 'download') downloadBlob(x.zpl, 'application/octet-stream', x.identificador + '.prn');
    const range = x.quantidade > 1 ? `${x.identificador} até ${x.ultimo_identificador}` : x.identificador;
    status(`${x.quantidade} etiqueta${x.quantidade > 1 ? 's' : ''} (${range}) ${dest === 'imprimir' ? 'enviada' + (x.quantidade > 1 ? 's' : '') : 'gerada'} com sucesso.`, 'ok');
    reportPeriods = [];
    document.querySelector('#reportYear').replaceChildren();
    document.querySelector('#reportMonth').replaceChildren();
  } catch (error) {
    status(error.message, 'error');
  } finally {
    if (attempted) {
      // Uma falha de rede ou da impressora também pode ter reservado números.
      try { await refreshStateAndPreview(); }
      catch (error) { showToast('Confira o contador antes de tentar novamente: ' + error.message, 'error'); }
      setBusy(false);
    }
...
let reportPeriods = [], reportLoadRevision = 0;
...
async function loadReportPeriods() {
  ...
  try {
    ...
    const periods = await response.json(); if (revision !== reportLoadRevision) return; reportPeriods = periods;
    ...
    if (revision === reportLoadRevision) showToast(error.message, 'error');
  }
}
